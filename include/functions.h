...
// ??? sub_801DD54(???);
// ??? Destruct6CBMXFADE(???);
void StartBMXFADE(s8 strongLock);
...
// ??? PutUnitSpriteForJid(???);
// ??? sub_8027CFC(???);
void sub_8027DB4(int layer, int x, int y, u16 oam2base, int jid, int id);
// ??? PutUnitSpriteExt(???);
// ??? sub_8027F28(???);
...
s8 AiUnitUpdateGetHealMode(struct Unit* unit);
s8 AiUnitTryHealSelf(void);
s8 AiUnitTryMoveTowardsEscapePoint(void);
...
// ??? sub_8079FA8(???);
// ??? MoveUnit_(???);
void GenUnitDefinitionFinalPosition(const struct UnitInfo* info, u8* xOut, u8* yOut, s8 findNearest);
// ??? sub_807A0E4(???);
// ??? sub_807A194(???);
// ??? MuCtr_OnEnd(???);
// ??? sub_807A294(???);
